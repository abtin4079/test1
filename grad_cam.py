...
def show_cams(img, mask_dic, image_number):
    for name, mask in mask_dic.items():
        show_cam_on_image(img, mask, name, image_number)
    
def show_cam_on_image(img, mask, name, image_number):
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
    heatmap = np.float32(heatmap) / 255
    cam = heatmap + np.float32(img)
    cam = cam / np.max(cam)
    cv2.imwrite(f"F:/UNIVERCITY/sharifian/DATASETS_FOR_COLAB/COVID_19_DATASET_INFECTION/colab/test/gradcam_eff/{image_number}.png", np.uint8(255 * cam))
